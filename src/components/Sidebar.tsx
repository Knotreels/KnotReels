    ...
    <>
      ...
      >
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <button className="text-white text-xl md:hidden" onClick={onClose} aria-label="Close Sidebar">
            <FaTimes />
          </button>
        </div>
        ...
        {!collapsed && (
          <Link href="/dashboard/orin">
            <div className="relative w-full rounded-xl overflow-hidden mb-4 shadow-lg group transition hover:shadow-[0_0_20px_4px_rgba(59,130,246,0.8)]">
              <img src="/orin-poster.jpg" alt="Tales of Orin" className="w-full h-40 object-cover" />
        ...
        <div className="flex-1 overflow-y-hidden hover:overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-zinc-900 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <button
              ...
              className="w-full p-2 bg-slate-700 hover:bg-slate-600 rounded text-white text-sm font-medium flex items-center justify-center gap-2"
            >
          ...
          {navSections.map(({ heading, items }) => (
            <div key={heading} className="space-y-2">
              ...
              {items.map(({ label, icon: Icon, href, external }) => {
                // force "Comic Books" to point at /comics
                const finalHref = label === 'Comic Books' ? '/comics' : href;
                ...
                if (label === 'AI Classes') {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-3 p-2 rounded hover:bg-zinc-800 transition ${
                        pathname === finalHref ? 'bg-blue-700 text-white' : 'text-gray-300'
                      }`}
                      onClick={onClose}
                    >
                      <Icon className="text-lg" />
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col gap-3 px-2 text-white">
              <a href="https://x.com/KnotReels" target="_blank" className="flex items-center gap-2 hover:text-white">
                <FaXTwitter className="w-5 h-5 text-white" />
                {!collapsed && <span className="text-sm">X</span>}
              </a>
              <a href="https://www.instagram.com/__knotreels__" target="_blank" className="flex items-center gap-2 hover:text-pink-500">
                <FaInstagram className="w-5 h-5 text-pink-500" />
                {!collapsed && <span className="text-sm">Instagram</span>}
              </a>
              <a href="https://facebook.com/yourhandle" target="_blank" className="flex items-center gap-2 hover:text-blue-500">
                <FaFacebook className="w-5 h-5 text-blue-500" />
                {!collapsed && <span className="text-sm">Facebook</span>}
              </a>
              <a href="https://www.tiktok.com/@knotreels" target="_blank" className="flex items-center gap-2 hover:text-white">
          ...
          <div className="mt-6">
            {!collapsed && (
              <Link href="/terms" className="flex items-center gap-2 text-xs text-gray-400 hover:text-white">
                Terms & Conditions
              </Link>
            )}
          </div>
          ...
          <div className="fixed inset-0 overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4">
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 shadow-xl transition-all">
                ...
                <div className="mt-4">
                  <Button onClick={() => setShowComingSoon(false)}>Got it, thanks!</Button>
                </div>
              </Dialog.Panel>
            </div>
          </div>
